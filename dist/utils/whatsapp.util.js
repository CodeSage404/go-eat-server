...
const sendWhatsApp = async (to, message) => {
    ...
    if (!accountSid || !authToken || !from) {
        logger_1.default.warn(`⚠️ [WhatsApp MOCK] Twilio not fully configured. Message: "${message}" to ${whatsappTo}`);
        return true;
    }
    try {
        ...
        if (response.status === 201) {
            logger_1.default.info(`📱 WhatsApp message sent successfully to ${whatsappTo} via Twilio`);
            return true;
        }
        else {
            logger_1.default.error('❌ Twilio WhatsApp sending failed:', response.data);
            return false;
        }
    }
    catch (error) {
        logger_1.default.error('❌ Error sending WhatsApp via Twilio:', error.response?.data || error.message);
        return false;
    }
};
