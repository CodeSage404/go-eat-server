...
class AuthController {
    constructor() {
        ...
        this.appleLogin = (0, catchAsync_1.catchAsync)(async (req, res) => {
            ...
            const result = await auth_service_1.default.socialLogin('apple', token, role || user_model_1.UserRole.CUSTOMER, name);
            res.status(200).json({
                status: 'success',
                token: result.token,
                data: { user: result.user },
        ...
        this.appleCallback = (0, catchAsync_1.catchAsync)(async (req, res) => {
            const payload = req.method === 'POST' ? req.body : req.query;
            const { id_token, user: userJson, state, error } = payload;
            let role = user_model_1.UserRole.CUSTOMER;
            ...
            if (state) {
                try {
                    ...
                    if (decodedState.clientRedirectUri)
                        clientRedirectUri = decodedState.clientRedirectUri;
                }
                catch (e) {
                    logger_1.default.warn('Failed to parse Apple state parameter:', e);
                }
            }
            if (error) {
            ...
            const result = await auth_service_1.default.socialLogin('apple', id_token, role, fullName);
