// Local library database of Nigeria States and their Local Government Areas (LGAs)
...
export const NIGERIA_STATES_DB: StateData[] = [
  {
    state: "Abia",
    alias: "abia",
...
export function getStates(): string[] {
  return NIGERIA_STATES_DB.map(s => s.state);
}

export function getLgasByState(stateName: string): string[] {
  const match = NIGERIA_STATES_DB.find(
    s => s.state.toLowerCase() === stateName.toLowerCase()
  );
  return match ? match.lgas : [];
}

export function searchNigeriaLocations(query: string): string[] {
  const q = query.toLowerCase().trim();
  const matches: string[] = [];
  
  for (const item of NIGERIA_STATES_DB) {
